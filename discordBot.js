...
async function processNextRequest(sogni) {
  ...
  try {
    ...
    if (error.message === 'Timeout exceeded: 30s') {
      channel.send('Your request took too long (over 30 seconds) and was canceled. Please try again.');
    } else if (error.message && error.message.includes('Insufficient funds')) {
      channel.send('Sorry, the bot is out of funds. Please request a top-up!');
    } else if (error && error.status === 401) {
      channel.send('Authentication error with Sogni API. The bot might need a restart.');
    } else {
      channel.send(`An error occurred: ${error.message || 'Unknown error'}. Please try again.`);
    }
  } finally {
